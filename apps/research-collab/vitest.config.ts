...
// Resolve harness-one subpath imports to their source so tests don't need a
// prior build step — mirrors the pattern used in apps/dogfood + packages/preset.
...
export default defineConfig({
  test: {
    ...
    coverage: {
      provider: 'v8',
      include: ['src/**/*.ts'],
      // bin is a CLI shell; live HTTP impls require network for meaningful
      // coverage and are exercised opportunistically in mock mode only.
