...
async function main() {
  ...
  // Returns a Promise as of harness-one 0.3 — see CHANGELOG `CheckpointStorage
  // async migration`. Pair with `createFsCheckpointStorage` for crash-safe
  // disk-backed persistence.
  const cp1 = await checkpoints.save(conversation, 'after-analysis');
  ...
  const cp2 = await checkpoints.save(conversation, 'after-report');
  ...
  const restored = await checkpoints.restore(cp1.id);
  ...
  const pruned = await checkpoints.prune({ maxCheckpoints: 1 });
