...
> 这些数字反映 Wave-25 时的 main 分支状态；git 随时变。若看文档时发现落差，
...
- **测试工具**: vitest（ESM 原生、TypeScript 优先）；Wave-25 baseline 4,486 用例
- **类型检查**: tsc --noEmit（strict 模式）
- **包格式**: 单核心包 `harness-one`，通过 `exports` 字段提供 14 个子路径（`core` / `advanced` / `prompt` / `context` / `tools` / `guardrails` / `observe` / `session` / `memory` / `evolve-check` / `rag` / `orchestration` / `redact` / `infra`）
...
| [14-advanced.md](./14-advanced.md) | Advanced 扩展点桶：middleware、StreamAggregator、SSE、FallbackAdapter、resilient loop、test utils |
