...
| 文件 | 职责 | 约行数 |
|------|------|--------|
| `src/orchestration/types.ts` | 类型定义：PoolConfig、HandoffPayload、BoundaryPolicy 等 | ~222 |
...
- **依赖**: `core/agent-loop.ts`（AgentLoop 类型）、`core/errors.ts`（HarnessError）、`orchestration/orchestrator.ts`（AgentOrchestrator 接口，仅 Handoff）
- **被依赖**: 无直接模块依赖
