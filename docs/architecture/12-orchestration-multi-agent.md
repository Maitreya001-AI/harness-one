...
`createBasicRoundRobinStrategy` / `createBasicRandomStrategy` /
`createBasicFirstAvailableStrategy` 是起步策略，不是生产最优调度器。生产环境通常
还需要队列深度、成本、角色能力、SLO 或租户隔离信号。
...
| `src/orchestration/strategies.ts` | `createBasicRoundRobinStrategy` / `createBasicRandomStrategy` / `createBasicFirstAvailableStrategy` | 89 |
