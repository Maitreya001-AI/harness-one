...
Wave-5B 之后 `AgentLoop` 从单体类分解为"协调骨架 + 若干专职子模块"。
列表按职责分组：
...
> / `createStreamingMockAdapter` / `createErrorStreamingMockAdapter`) now
> live in `src/testing/test-utils.ts` and ship on the `harness-one/testing`
> subpath — see [`17-testing.md`](./17-testing.md). Prior to Wave-27 they
> sat in `src/core/test-utils.ts` and were re-exported from
> `harness-one/advanced`; the split stops them from masquerading as
...
> **历史变更**：Wave-5B 的 AgentLoop 模块分解、Wave-5H 架构加固、Wave-8 /
> Wave-15（`HarnessError` / `HarnessErrorCode` 下沉到 L1）/ Wave-23 / Wave-25
> 等详细记录见仓库根目录的 [`MIGRATION.md`](../../MIGRATION.md) 与 `git log`。
> 本文件只维护当前的稳定参考。
...
`AgentLoop.run()` 是一个 AsyncGenerator，Wave-5B 后 `run()` 自身仅
