...
| `validate(id, context?)` | 验证组件假设——评估退役条件。字符串条件使用简单 DSL（`key operator value`，支持 `>`, `<`, `>=`, `<=`, `==`, `!=`）；函数条件直接以 context 对象调用。条件满足时返回 `{ valid: false, reason }`。未提供 context 或无退役条件时返回 valid |
...
- `ComponentMeta.retirementCondition` 支持字符串 DSL 和函数两种形式，字符串形式使用 `key operator value` 格式，函数形式接收 context 对象返回 boolean
