...
> Property-based fuzz 套件，覆盖 4 个高风险 parser。和常规单测 + PBT 分开
> 跑，避免把 ~10k 次迭代塞进 PR 关键路径。PBT 和 fuzz 都基于 `fast-check`
> 但侧重不同：PBT 验证已知性质（幂等性、边界），fuzz 验证幸存性
> （任何输入都不能 uncaught throw）。
...
每个 target 都配一个 `tests/fuzz/corpus/<target>/` 目录，里面存 10–15 个
...
复现步骤：把 `seed: <n>` 塞到 `fc.assert(property, { seed: <n> })` 里，
或者对整个文件 `FC_SEED=<n> pnpm --filter harness-one fuzz -- <file>
--reporter=verbose`。
...
真正的 bug 走 coordinated disclosure 流程（见 `SECURITY.md`），跟踪清单：
`packages/core/tests/fuzz/FINDINGS.md`。历史条目：

- `F-O4-01`（已修复）— `createPromptRegistry().resolve()` 对
  `Object.prototype` 上的键名（`toString` / `valueOf` / `constructor`
  / …）走 `in`-操作符 fallback，抛 `TypeError` 而非 `HarnessError`。
  修复：`Object.prototype.hasOwnProperty.call` 显式 own-property 检查；
  回归覆盖在 `src/prompt/__tests__/registry.test.ts` 的
  `F-O4-01` describe。
