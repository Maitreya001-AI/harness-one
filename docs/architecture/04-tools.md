...
**默认配额**：`createRegistry()` 无参数时默认 `maxCallsPerTurn=20`, `maxCallsPerSession=100`,
...
| 文件 | 职责 | 约行数 |
|------|------|--------|
| `src/tools/types.ts` | 类型定义 + `toolSuccess`/`toolError` 辅助函数 + `ToolCapability` 枚举（含 `ALL_TOOL_CAPABILITIES`） | 244 |
| `src/tools/define-tool.ts` | `defineTool` 工厂——创建冻结的 `ToolDefinition` | 108 |
| `src/tools/registry.ts` | `createRegistry` + `createPermissiveRegistry` 工厂——注册、验证、执行、速率限制、capability 过滤、middleware 组合 | 552 |
| `src/tools/validate.ts` | `validateToolCall`——委托 `infra/json-schema.ts` | 35 |
| `src/tools/index.ts` | 公共导出桶文件 | 26 |
...
| `ToolMiddleware<TParams>` | `(ctx, next) => Promise<ToolResult>` — 横切关注点的洋葱包装器 |
...
- **依赖**: `core/types.ts`（JsonSchema、ToolCallRequest、ToolSchema）、`core/errors.ts`（HarnessError）、`infra/json-schema.ts`
- **被依赖**: 无直接模块依赖
