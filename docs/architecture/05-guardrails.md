...
| `PipelineResult` | Pipeline 执行结果：passed、verdict、results |
...
护栏按数组顺序串行执行。遇到 `block` 立即返回（短路）。遇到 `modify` 也立即返回但 `passed = true`。所有 `allow` 则最终返回 allow。
...
- Pipeline 不支持并行执行护栏
- 自愈循环中 modify 判定也视为失败（收集 reason），不使用修改后的内容
