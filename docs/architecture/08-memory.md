...
ContextRelay 将 RelayState 序列化为 JSON 存入 MemoryStore（key 为 `__relay__`，grade 为 `critical`）。通过 `findRelay()` 内部方法查找，优先使用缓存的 currentId，否则通过 query 搜索。JSON.parse 调用包裹在 try-catch 中——损坏的 relay 数据静默返回 null，不发出 `console.warn`，也不抛出未处理的 SyntaxError。
