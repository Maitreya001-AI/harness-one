...
| 文件 | 职责 | 约行数 |
|------|------|--------|
| `src/memory/types.ts` | 类型定义：`MemoryEntry`、`MemoryFilter`、`CompactionPolicy`、`RelayState`、`VectorSearchOptions` | 83 |
| `src/memory/store.ts` | `MemoryStore` 接口 + `MemoryStoreCapabilities` + `createInMemoryStore` | 538 |
| `src/memory/memory-query.ts` | query filter 实现（grade / tags / since / search / limit / offset） | 103 |
| `src/memory/fs-io.ts` | 文件 I/O 原语（读/写/批量/索引），`withIndexLock` 互斥锁，含 schema 校验 | 261 |
| `src/memory/fs-store.ts` | `createFileSystemStore`——文件系统后端；atomic write 和 `reconcileIndex()` | 435 |
| `src/memory/relay.ts` | `createRelay`——跨上下文接力；`dispose()` 释放缓存 | 291 |
| `src/memory/_schemas.ts` | JSON 反序列化边界的 schema 校验：`validateMemoryEntry` / `validateIndex` / `validateRelayState` / `parseJsonSafe` | 228 |
| `src/memory/testkit.ts` | `runMemoryStoreConformance` — 框架无关的合规套件 | 189 |
| `src/memory/index.ts` | 公共导出桶文件 | 48 |
...
JSON.parse 后的内容会经 `validateRelayState()` 做 shape 校验；shape 不匹配与 JSON 解析失败都会通过 `onCorruption` 回调上报并跳过，不再静默当作空 relay 处理。
...
1. **ContextRelay 资源释放**：`ContextRelay` 提供 `dispose()` 方法，用于清理内部缓存和引用，防止长运行服务中的内存泄漏。
2. **向量搜索 top-K 优化**：`searchByVector()` 使用有界 top-K 选择算法（O(N·K) 复杂度），而非全排序（O(N log N)），在 K 远小于 N 的典型检索场景下显著降低计算开销。
