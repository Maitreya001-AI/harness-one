...
| `src/observe/failure-taxonomy.ts` | `createFailureTaxonomy`——6 个内置检测器（tool_loop / early_stop / budget_exceeded / timeout / repeated_tool_failure / adapter_retry_storm） | 270 |
...
| `thresholds.adapterRetryStormMinErrors` | `number` | `3` | 触发 `adapter_retry_storm` 所需的最小可重试错误 Span 数 |
...
| `adapter_retry_storm` | ≥3 个 error Span 携带可重试 `harness.error.code`（`ADAPTER_RATE_LIMIT` / `ADAPTER_NETWORK` / `ADAPTER_UNAVAILABLE` / `ADAPTER_CIRCUIT_OPEN` / `CORE_TIMEOUT`），`details.codeHistogram` 给出分布 | 0.6 + (count - 3) × 0.05，上限 0.9 |
...
### span 级 error 分类 vs trace 级 failure taxonomy

这两个子系统职责正交，请不要互相替代：

| 维度 | `core/error-span-attributes.ts` | `observe/failure-taxonomy.ts` |
|------|--------------------------------|-------------------------------|
| 粒度 | 单个 span | 完整 trace |
| 输入 | 一个 `HarnessError` 实例 | 一条完成的 `Trace`（含所有 span） |
| 输出 | 给 span 写入 `harness.error.code` + `harness.error.retryable` 属性 | 返回按置信度排序的 `FailureClassification[]` |
| 用途 | OTel exporter / Langfuse 等把单条错误上报时富化字段 | SRE 事后分析一次 agent 运行的失败模式（为什么它没完成？） |
| 写入时机 | `annotateHarnessErrorSpan()` 在错误捕获点同步调用 | `classify()` 在 trace 结束后任意时刻调用 |

`isRetryableHarnessErrorCode()` 是两个子系统之间唯一明确的桥梁：
`adapter_retry_storm` 检测器读取 span 上由 `annotateHarnessErrorSpan()` 写
入的 `harness.error.code`，用同一个可重试码集合判定是否为 adapter 抖动。
这样 span 级富化和 trace 级分类共用一份可重试定义，不会漂移。

写自定义检测器时，**优先消费 `harness.error.code` 等结构化属性**，不要
重新实现错误码匹配逻辑——直接从 `harness-one/advanced` 拿
`isRetryableHarnessErrorCode` 或比较 `HarnessErrorCode` 常量。
