...
**事件丢弃可见性**：当 pendingEvents 队列达到 `MAX_PENDING_EVENTS` 上限时，新事件被丢弃。`droppedEvents` 只读属性（getter）暴露累计丢弃计数，首次丢弃时通过 logger 发出警告（boolean latch 防止日志风暴）。
...
1. **ConversationStore.clear()**：`ConversationStore` 接口提供可选的 `clear()` 方法，内存实现 `createInMemoryConversationStore()` 已提供实现，用于清空所有会话的消息历史。
2. **Session list() 重入安全**：`list()` 方法先收集所有过期会话，再在迭代结束后统一触发过期事件，避免在 Map 迭代过程中因事件回调修改 Map 导致的重入问题。
...
interface ConversationStore {
  ...
  list(): Promise<string[]>;
  clear?(): Promise<void>;  // 可选，清空所有会话消息历史
}
